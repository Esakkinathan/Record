...
class DonutChartView: UIView {
    ...
    // MARK: Trait change registration (iOS 17+ API, no deprecation warning)

    private func registerTraitChanges() {
        if #available(iOS 17.0, *) {
            registerForTraitChanges([UITraitUserInterfaceStyle.self]) { (view: DonutChartView, _) in
    ...
    // MARK: Fallback for iOS 16 and below

    override func traitCollectionDidChange(_ previousTraitCollection: UITraitCollection?) {
        super.traitCollectionDidChange(previousTraitCollection)

        // Only run on iOS 16 and below; iOS 17+ is handled via registerForTraitChanges
    ...
    override func draw(_ rect: CGRect) {
        super.draw(rect)
        guard let container = donutContainerView else { return }
        ...
        // ── Empty state: all segments are 0 or segments is empty ──────────────
        let activeSegments = segments.filter { $0.value > 0 }
        let isEmpty = total <= 0 || activeSegments.isEmpty

        if isEmpty {
            // Draw a single full-circle track ring in a muted color
            ...
            emptyLayer.strokeColor = UIColor.barTrack.resolvedColor(with: traitCollection).cgColor
            emptyLayer.fillColor   = UIColor.clear.cgColor
            emptyLayer.lineWidth   = strokeWidth
            emptyLayer.lineCap     = .butt
            container.layer.addSublayer(emptyLayer)
        ...
        // ── Normal state ───────────────────────────────────────────────────────
        // Gap between slices (radians). Only applied when more than one slice.
        let gap: CGFloat = activeSegments.count > 1 ? 0.03 : 0.0
        var startAngle: CGFloat = -.pi / 2
        ...
        for segment in activeSegments {
            ...
            shapeLayer.fillColor   = UIColor.clear.cgColor
            shapeLayer.lineWidth   = strokeWidth
            // .butt gives sharp flat ends — the gap between arcs acts as the separator
